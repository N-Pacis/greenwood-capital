...
export default function ScoreDashboard() {
  ...
  return (
    <div className="space-y-8">
      {/* Welcome Section */}
      <div className="bg-primary rounded-2xl p-6 text-white font-heading">
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1">
          <div className="bg-card rounded-2xl p-6 shadow-sm border border-border">
            <h2 className="text-lg font-semibold text-main text-center mb-6 font-heading">
        ...
        <div className="lg:col-span-2">
          <div className="bg-card rounded-2xl p-6 shadow-sm border border-border h-full">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-main font-heading">How Your Score is Calculated</h2>
              <button
            ...
            {/* Weights breakdown */}
            {showWeightsInfo && (
              <div className="bg-panel rounded-lg p-4 mb-4 border border-border">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {Object.entries(SCORING_CONFIG.weights).map(([key, weight]) => (
                    <div key={key} className="flex items-center justify-between bg-card rounded px-3 py-2 border border-border">
                      <span className="text-sm text-main">{SCORING_CONFIG.factorLabels[key]}</span>
                      <span className="text-sm font-medium text-primary">{formatWeight(weight)}</span>
                    </div>
                  ))}
                </div>
            ...
            <div className="flex items-center gap-2 text-sm text-muted mb-4 overflow-x-auto pb-2">
              <span className="font-medium text-main">Formula:</span>
              {Object.entries(SCORING_CONFIG.weights).map(([key, weight], index) => (
                <span key={key} className="flex items-center gap-1 whitespace-nowrap">
                  ...
                  <span className="bg-panel px-2 py-1 rounded border border-border">
            ...
            <div className="flex items-center justify-between mt-4 pt-4 border-t border-border">
              <span className="font-medium text-main">Total Score</span>
              <span className="text-xl font-bold text-primary">{scoreResult.totalScore}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Factor Cards */}
      <div>
        <h2 className="text-lg font-semibold text-main mb-4 font-heading">Score Breakdown by Factor</h2>
      ...
      <div className="bg-panel rounded-xl p-6 border border-border">
        <div className="flex items-start gap-4">
          ...
          <div>
            <h3 className="font-semibold text-main mb-1 font-heading">Our Commitment to Transparency</h3>
